#
# twisted_sphinx.py 22.04.2010 18:45 GMT
#
# Python version of Sphinx searchd protocol
#
# Copyright (c) 2010, Andrew Nikishaev
...
class Sphinx(protocol.Protocol, policies.TimeoutMixin):
    ...
    def dataReceived(self, data):
        ...
        if len(data) == 4:
            ...
            if v[0]<1:
                ...
                print 'expected searchd protocol version, got %s' % v
                return
    ...
    def _replyReceived(self,data):
        ...
        if not response or read!=length:
            ...
            self._rawReplyQueue.put(None)

        # check status
        if status==SEARCHD_WARNING:
            wend = 4 + unpack ( '>L', response[0:4] )[0]
            self._warning = response[4:wend]
            self._rawReplyQueue.put(response[wend:])
        ...
        self._rawReplyQueue.put(response)
    ...
    def RunQueries (self):
        ...
        response = yield self._rawReplyQueue.get()
    ...
    def UpdateAttributes ( self, index, attrs, values ):
        ...
        if not response:
            self._replyQueue.put(-1)
            return

        # parse response
        updated = unpack ( '>L', response[0:4] )[0]
        self._replyQueue.put(updated)
